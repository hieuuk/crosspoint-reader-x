...
bool BookMetadataCache::buildBookBin(const std::string& epubPath, const BookMetadata& metadata) {
  ...
  const uint32_t metadataSize = metadata.title.size() + metadata.author.size() + metadata.language.size() +
                                ...
                                metadata.identifier.size() + metadata.syncUrl.size() + metadata.bookPosition.size() +
                                sizeof(uint32_t) * 8;
  ...
  serialization::writeString(bookFile, metadata.syncUrl);
  serialization::writeString(bookFile, metadata.bookPosition);
...
bool BookMetadataCache::load() {
  ...
  serialization::readString(bookFile, coreMetadata.syncUrl);
  serialization::readString(bookFile, coreMetadata.bookPosition);
